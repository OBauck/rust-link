...
async fn dap_task(
    ...
    led_yellow_pin: Output<'static>,
    led_red_pin: Output<'static>,
) -> ! {
    ...
    let leds = MyLeds {
        led_connected: led_red_pin,
        led_running: led_yellow_pin,
    };

    let context = Context::from_pins(swdio, swclk, nreset, CPU_FREQUENCY, bit_bang_delay);
    let swo: Option<Swo> = None;

    let mut dap_handler = Dap::new(context, leds, wait, swo, "V2");
...
async fn main(spawner: Spawner) {
    ...
    config.product = Some("OB-Link-STM32G0B1 (CMSIS-DAP v2)"); // Need to have "CMSIS-DAP" in the product name to make probe-rs recognize it
    ...
    spawner
        .spawn(dap_task(
            ...
            Flex::new(p.PA3),
            Flex::new(p.PA1),
            Output::new(p.PA5, Level::High, Speed::High),
...
struct MyLeds<'a> {
    led_connected: Output<'a>,
    led_running: Output<'a>,
}
    ...
    fn react_to_host_status(&mut self, host_status: HostStatus) {
        match host_status {
            HostStatus::Connected(val) => match val {
                true => self.led_connected.set_low(),
                false => self.led_connected.set_high(),
            },
            HostStatus::Running(val) => match val {
                true => self.led_running.set_low(),
                false => self.led_running.set_high(),
            },
        }
    }
}
